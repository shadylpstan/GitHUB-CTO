...
export class OpenAIPlanner {
  constructor({ apiKey, model, timeoutMs = 180000, maxRetries = 2, maxContextCharsPerFile = 12000 }) {
    this.apiKey = apiKey;
    this.model = model;
    this.timeoutMs = timeoutMs;
    this.maxRetries = maxRetries;
    this.maxContextCharsPerFile = maxContextCharsPerFile;
      ...
      messages: [
        { role: "system", content: this.systemPrompt },
        {
          ...
          content: buildUserPrompt({
            ...
            triage,
            contextFiles,
            maxContextCharsPerFile: this.maxContextCharsPerFile,
          }),
        },
      ],
    });
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt += 1) {
      ...
      try {
        ...
        if (response.status === 429 || response.status >= 500) {
          ...
          lastError = new Error(`OpenAI API ${response.status}: ${payload.error?.message || JSON.stringify(payload)}`);
...
function buildUserPrompt({ issue, comments, triage, contextFiles, maxContextCharsPerFile }) {
  ...
  const files = contextFiles
    .map((file) => {
      return [
        ...
        file.summary,
        "# Content",
        trim(file.content, maxContextCharsPerFile),
        "--- END FILE ---",
      ].join("\n");
    })
    .join("\n\n");
